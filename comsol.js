...
function stickySlideDesktop() {
  ...
  const clipTrigger = ScrollTrigger.create({
    ...
    start: "top top",
    end: `bottom+=${window.innerHeight * 0.4} bottom`,
    onUpdate: (self) => {
      updateImageHeights(imgs, self.progress);
    },
  });
  ...
  const slideAnim = gsap
    .timeline({
      scrollTrigger: {
        trigger: ".comsol-stickyslider .sticky-slide",
        start: "top 10%",
        pin: true,
        end: `bottom+=${window.innerHeight * 0.9} bottom`,
        scrub: true,
        onLeave: () => {
          gsap.to(".comsol-stickyslider .scroll-directive", {
            opacity: 0,
            duration: 0.3,
          });
        },
